...
void format_buffer(char *buffer){
    ...
    for(i = 0; buffer[i]; i++) {
        if(buffer[i] == 0x7f) {
            if(j > 0) j--;  // remove previous character
        } else {
            temp[j++] = buffer[i];
        }
    }
...
int main(void)
{
  ...
  char buffer[4095];
  memcpy(buffer,'\0',sizeof(char)*4095);
